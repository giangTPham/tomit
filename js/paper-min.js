var paper = new function() {
        ...
        return inject(function() {}, {
            inject: function(src) {
                if (src) {
                    ...
                    if (statics != src)
                        inject(proto, src, src.enumerable, base && base.prototype, src.preserve, src.generics && this);
    ...
    var Rectangle = this.Rectangle = Base.extend({
        ...
        return Base.each([["Top", "Left"], ["Top", "Right"], ["Bottom", "Left"], ["Bottom", "Right"], ["Left", "Center"], ["Top", "Center"], ["Right", "Center"], ["Bottom", "Center"]], function(parts, index) {
            var part = parts.join("");
    ...
    var Matrix = this.Matrix = Base.extend({
        ...
        _inverseTransform: function(point, dest, dontNotify) {
            ...
            if (!dest)
                dest = new Point(Point.dont);
            return dest.set((x * this._d - y * this._b) / det, (y * this._a - x * this._c) / det, dontNotify)
        },
    ...
    , {
        ...
        hitTest: function(point, options, matrix) {
            options = HitResult.getOptions(point, options);
            point = options.point;
            if (!this._children && !this.getRoughBounds(matrix).expand(options.tolerance)._containsPoint(point))
                return null;
            if ((options.center || options.bounds) && !(this instanceof Layer && !this._parent)) {
                var bounds = this.getBounds(), that = this, points = ["TopLeft", "TopRight", "BottomLeft", "BottomRight", "LeftCenter", "TopCenter", "RightCenter", "BottomCenter"], res;
        ...
        insertAbove: function(item) {
            return item._parent && item._parent.insertChild(item._index + 1, this)
        },
        ...
        transform: function(matrix, flags) {
            ...
            if (bounds && matrix.getRotation() % 90 === 0) {
                this._bounds = this._createBounds(matrix._transformBounds(bounds));
    ...
    var Raster = this.Raster = PlacedItem.extend({
        ...
        _calculateBounds: function(getter, matrix) {
            return matrix._transformBounds((new Rectangle(this._size)).setCenter(0, 0))
        },
        getHandleBounds: function() {
    ...
    var Segment = this.Segment = Base.extend({
        initialize: function(arg0, arg1, arg2, arg3, arg4, arg5) {
            var count = arguments.length, createPoint = SegmentPoint.create, point, handleIn, handleOut;
            if (count == 0)
                ;
            else if (count == 1)
                if (arg0.point) {
    ...
    var SegmentPoint = Point.extend({
        ...
        statics: {
            create: function(segment, key, pt) {
                var point = new SegmentPoint(SegmentPoint.dont), x, y, selected;
                if (!pt)
                    x = y = 0;
        ...
        getCrossings: function(point, matrix, roots) {
            ...
            for (var i = 0; i < num; i++) {
                ...
                if (t >= 0 && t < 1 && Curve.evaluate(vals, t, 0).x > point.x) {
                    if (t < Numerical.TOLERANCE && Curve.evaluate(this.getPrevious().getValues(matrix), 1, 1).y * Curve.evaluate(vals, t, 1).y >= 0)
                        continue;
                    crossings++
                }
            }
        ...
        statics: {
            ...
            getValues: function(segment1, segment2, matrix) {
                var p1 = segment1._point
                  ...
                  , p2 = segment2._point
                  , coords = [p1._x, p1._y, p1._x + h1._x, p1._y + h1._y, p2._x + h2._x, p2._y + h2._y, p2._x, p2._y];
            ...
            getParameter: function(v, x, y) {
                var txs = [], tys = [], sx = Curve.solveCubic(v, 0, x, txs), sy = Curve.solveCubic(v, 1, y, tys), tx, ty;
    ...
    , new function() {
        ...
        var zCubic = [[1, 0.6, 0.3, 0.1], [0.4, 0.6, 0.6, 0.4], [0.1, 0.3, 0.6, 1]];
        ...
        return {
            getNearestLocation: function(point, matrix) {
                ...
                var minDist = Infinity, minT, minPoint;
    ...
    var Path = this.Path = PathItem.extend({
        ...
        setClosed: function(closed) {
            if (this._closed != (closed = !!closed)) {
                this._closed = closed;
                if (this._curves) {
                    var length = this._segments.length, i;
                    if (!closed && length > 0)
                        length--;
                    this._curves.length = length;
                    if (closed)
                        this._curves[i = length - 1] = Curve.create(this, this._segments[i], this._segments[0])
                }
                this._changed(Change.GEOMETRY)
            }
        },
        ...
        _hitTest: function(point, options, matrix) {
            var tolerance = options.tolerance || 0, radius = (options.stroke ? this.getStrokeWidth() / 2 : 0) + tolerance, loc, res;
            var coords = []
              , that = this;
            function checkSegment(segment, ends) {
                ...
                for (var j = ends || options.segments ? 0 : 2, m = !ends && options.handles ? 6 : 2; j < m; j += 2)
                    if (point.getDistance(coords[j], coords[j + 1]) < tolerance)
                        return new HitResult(j == 0 ? "segment" : "handle-" + (j == 2 ? "in" : "out"),that,{
                            segment: segment
                        })
            }
        ...
        function drawSegments(ctx, path) {
            var segments = path._segments, length = segments.length, handleOut, outX, outY;
            function drawSegment(i) {
                ...
                if (!handleOut)
                    ctx.moveTo(x, y);
                else if (handleIn.isZero() && handleOut.isZero())
                    ctx.lineTo(x, y);
                else
        ...
        function drawDashes(ctx, path, dashArray, dashOffset) {
            var flattener = new PathFlattener(path), from = dashOffset, to, i = 0;
    ...
    , new function() {
        ...
        return {
            ...
            smooth: function() {
                var segments = this._segments, size = segments.length, n = size, overlap;
                if (size <= 2)
                    return;
                if (this._closed) {
                ...
                if (this._closed) {
                    for (var i = 0, j = size; i < overlap; i++,
                    j++) {
                        var f1 = i / overlap;
                        var f2 = 1 - f1;
                        x[j] = x[i] * f1 + x[j] * f2;
                ...
                for (var i = overlap; i <= n - overlap; i++) {
                    ...
                    if (i < n) {
                        ...
                        if (i < n - 1)
                            handleIn = new Point(2 * knots[i + 1]._x - x[i + 1],2 * knots[i + 1]._y - y[i + 1]);
                        else
                            handleIn = new Point((knots[n]._x + x[n - 1]) / 2,(knots[n]._y + y[n - 1]) / 2)
                    }
                }
    ...
    , new function() {
        ...
        return {
            ...
            arcTo: function(to, clockwise) {
                var current = getCurrentSegment(this), from = current._point, through;
    ...
    Path.inject({
        statics: new function() {
            ...
            var ovalSegments = [new Segment([0, 0.5],[0, kappa],[0, -kappa]), new Segment([0.5, 0],[-kappa, 0],[kappa, 0]), new Segment([1, 0.5],[0, -kappa],[0, kappa]), new Segment([0.5, 1],[kappa, 0],[-kappa, 0])];
            return {
                Line: function() {
                ...
                Rectangle: function(rect) {
                    ...
                    path = new Path;
                    path._add([new Segment(Point.create(left, bottom)), new Segment(Point.create(left, top)), new Segment(Point.create(right, top)), new Segment(Point.create(right, bottom))]);
                    path._closed = true;
                    return path
                },
    ...
    var PathFlattener = Base.extend({
        initialize: function(path) {
            ...
            this.length = 0;
            this.index = 0;
            var segments = path._segments, segment1 = segments[0], segment2, that = this;
    ...
    var PathFitter = Base.extend({
        initialize: function(path, error) {
            this.points = [];
            var segments = path._segments, prev;
            for (var i = 0, l = segments.length; i < l; i++) {
        ...
        fitCubic: function(first, last, tan1, tan2) {
            ...
            var uPrime = this.chordLengthParameterize(first, last), maxError = Math.max(this.error, this.error * this.error), error, split;
            for (var i = 0; i <= 4; i++) {
        ...
        statics: {
            ...
            extend: function(src) {
                ...
                src._owner.inject(Base.each(src._defaults, function(value, key) {
                    ...
                    ;
                    src[get] = function() {
                        var children = this._item && this._item._children, style;
                        if (!children)
                            return this["_" + key];
                        for (var i = 0, l = children.length; i < l; i++) {
    ...
    var Color = this.Color = Base.extend(new function() {
        ...
        var converters = {
            ...
            "hsl-rgb": function(color) {
                var s = color._saturation, h = color._hue / 360, l = color._lightness, t1, t2, c;
                if (s == 0)
        ...
        toCssString: function() {
            if (!this._cssString) {
                var color = this.convert("rgb")
                  , alpha = color.getAlpha()
                  , components = [Math.round(color._red * 255), Math.round(color._green * 255), Math.round(color._blue * 255), alpha != null ? alpha : 1];
    ...
    DomEvent.requestAnimationFrame = new function() {
        ...
        var callbacks = [], focused = true, timer;
        DomEvent.add(window, {
            focus: function() {
                focused = true
            },
    ...
    var View = this.View = PaperScopeItem.extend({
        ...
        getBounds: function() {
            if (!this._bounds)
                this._bounds = this._getInverse()._transformBounds(new Rectangle(new Point,this._viewSize));
            return this._bounds
        },
        getSize: function() {
        ...
        setOnFrame: function(onFrame) {
            ...
            var that = this, requested = false, before, time = 0, count = 0;
    ...
    var Key = this.Key = new function() {
        ...
        DomEvent.add(document, {
            ...
            keyup: function(event) {
                var code = event.which || event.keyCode, key = keys[code], name;
    ...
    var BlendMode = {
        process: function(blendMode, srcContext, dstContext, alpha, offset) {
            var srcCanvas = srcContext.canvas, dstData = dstContext.getImageData(offset.x, offset.y, srcCanvas.width, srcCanvas.height), dst = dstData.data, src = srcContext.getImageData(0, 0, srcCanvas.width, srcCanvas.height).data, min = Math.min, max = Math.max, abs = Math.abs, sr, sg, sb, sa, br, bg, bb, ba, dr, dg, db;
        ...
        var parse_js = new function() {
            ...
            function S(a, b) {
                ...
                function u(a, b) {
                    for (var d = [], e = a.length - 1, f = 0; f <= e; ++f) {
                        var g = a[f]
                          , i = z(g);
                        i != ";" && (!c && f == e && (g[0] == "while" && O(g[2]) || L(g[0], ["for", "for-in"]) && O(g[4]) || g[0] == "if" && O(g[2]) && !g[3] || g[0] == "if" && g[3] && O(g[3]) ? i = i.replace(/;*\s*$/, ";") : i = i.replace(/;+\s*$/, "")),
                        d.push(i))
                    }
                    return b ? d : W(d, h)
                }
                ...
                function q(a) {
                    ...
                    Math.floor(a) === a ? (c.push("0x" + a.toString(16).toLowerCase(), "0" + a.toString(8)),
                ...
                function k(a) {
                    ...
                    for (var d = 0; d < a.length; ++d) {
                        var e = a[d + 1];
                        b.push(a[d]),
                        e && (/[a-z0-9_\x24]$/i.test(a[d].toString()) && /^[a-z0-9_\x24]/i.test(e.toString()) || /[\+\-]$/.test(a[d].toString()) && /^[\+\-]/.test(e.toString())) && b.push(" ")
                    }
                    return b.join("")
                }
                function j(a, b) {
                ...
                function h(a) {
                    a == null && (a = ""),
                    c && (a = T(" ", b.indent_start + d * b.indent_level) + a);
                    return a
                }
                function g(a) {
                    return a.toString()
                ...
                var c = !!b.beautify
                  ...
                  , r = {
                    ...
                    "try": function(a, b, c) {
                        var d = ["try", w(a)];
                        b && d.push("catch", "(" + b[0] + ")", w(b[1])),
                        c && d.push("finally", w(c));
                        return k(d)
                    },
                    "throw": function(a) {
                        return k(["throw", z(a)]) + ";"
                    ...
                    "for": function(a, b, c, d) {
                        var e = ["for"];
                        a = (a != null ? z(a) : "").replace(/;*\s*$/, ";" + f),
                        b = (b != null ? z(b) : "").replace(/;*\s*$/, ";" + f),
                        c = (c != null ? z(c) : "").replace(/;*\s*$/, "");
                        var g = a + b + c;
                        g == "; ; " && (g = ";;"),
                        e.push("(" + g + ")", z(d));
                        return k(e)
            ...
            function F(a, b, c) {
                ...
                function _() {
                    ...
                    while (!e("punc", "}")) {
                        ...
                        var f = d.token.type
                          , h = ba();
                        f != "name" || h != "get" && h != "set" || !!e("punc", ":") ? (m(":"),
                        c.push([h, bj(!1)])) : c.push([bb(), P(!1), h])
                    }
                    g();
                    return p("object", c)
                }
                ...
                function I(a) {
                    ...
                    b != null ? (g(),
                    L(b, d.labels) || i("Label " + b + " without matching loop or statement")) : d.in_loop == 0 && i(a + " not inside a loop or switch"),
                    o();
                    return p(a, b)
                }
                function F() {
                ...
                function g() {
                    d.prev = d.token,
                    d.peeked ? (d.token = d.peeked,
                    d.peeked = null) : d.token = d.input();
                    return d.token
                }
                function f() {
            ...
            function x(b) {
                ...
                function K() {
                    ...
                    switch (g()) {
                    case "/":
                        f.comments_before.push(G()),
                        f.regex_allowed = a;
                        return P();
                    case "*":
                        f.comments_before.push(H()),
                        f.regex_allowed = a;
                        return P()
                    }
                ...
                function G() {
                    h();
                    var a = t("\n"), b;
                    a == -1 ? (b = f.text.substr(f.pos),
                    f.pos = f.text.length) : (b = f.text.substring(f.pos, a),
                    f.pos = a);
                ...
                function E(a) {
                    ...
                    for (; a > 0; --a) {
                        ...
                        isNaN(c) && B("Invalid hex-character pattern in string"),
                        b = b << 4 | c
                    }
                    return b
                }
                ...
                function x(a, b, d) {
                    ...
                    d || (e.comments_before = f.comments_before,
                    f.comments_before = []),
                    f.newline_before = !1;
                    return e
                }
            ...
            var w = {}
              ...
              , A = function(a, b, c) {
                while (c < a.length)
                    b[a[c]] = a[c].substr(0, a[c].length - 1),
                    c++;
                return b
            }(["+=", "-=", "/=", "*=", "%=", ">>=", "<<=", ">>>=", "|=", "^=", "&="], {
                "=": !0
        ...
        function compile(code) {
            ...
            ast = walker.with_walkers({
                "binary": function(operator, left, right) {
                    return handleOperator(operator, left = walk(left), right = walk(right)) || [this[0], operator, left, right]
                },
                ...
                "unary-prefix": function(operator, exp) {
                    if (signOperators[operator] && isDynamic(exp))
                        return ["call", ["name", "$sign"], [["string", operator], walk(exp)]]
                }
            }, function() {
                return walk(ast)
            });
